...
class GroupAdminSeasonController extends Controller
{
    ...
    public function store(Request $request, Group $group)
    {
        ...
        $isAlreadyExists = Season::where('group_id', $group->id)->where('seasonable_id', $validatedData['serie']['id'])->where('seasonable_type', $isAnime ? Anime::class : Manga::class)->where('season_number', $validatedData['season_number'])->exists();
        if ($isAlreadyExists) {
        ...
        Season::create([
            ...
            'seasonable_type' => $isAnime ? Anime::class : Manga::class
        ]);

        if ($isAnime) {
            $anime = Anime::find($validatedData['serie']['id']);
            return redirect(route('group.admin.animes.edit', ['group' => $group, 'anime' => $anime, 'tab' => 'seasons']))->with('success', 'New Season Created');
        }

        return redirect(route('group.admin.mangas.edit', ['group' => $group, 'manga' => $validatedData['serie']['id'], 'tab' => 'seasons']))->with('success', 'New Season Created');
    }
    ...
    public function seasonUpdate(Group $group, Season $season)
    {
        ...
        if ($type === 'anime') {
            $anime = Anime::find($validatedData['serie']['id']);
            return redirect(route('group.admin.animes.edit', ['group' => $group, 'anime' => $anime, 'tab' => 'seasons']))->with('success', 'Season Updated');
        }

        return redirect(route('group.admin.mangas.edit', ['group' => $group, 'manga' => $validatedData['serie']['id'], 'tab' => 'seasons']))->with('success', 'Season Updated');
    }
