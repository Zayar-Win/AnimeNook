...
const DeleteModal = ({title,deleteHandler,setIsDeleteModalOpen}) => {
    return (
        <div className='fixed flex items-center justify-center top-0 left-0 w-full h-full bg-[rgba(0,0,0,0.4)]'>
            <div className='w-[40%] rounded-lg gap-3 py-8 flex flex-col items-center text-black bg-white'>
                ...
                <div className='w-[60%]'>
                    <Button type={'button'} onClick={() => setIsDeleteModalOpen(false)} className={'!text-black w-full'} outline text={'Cancel'} />
                </div>
                <div className='w-[60%]'>
                    <Button type={'button'} onClick={deleteHandler} className={'!bg-red-500 w-full'} text={'Delete'} />
                </div>
            </div>
        </div>
    )
