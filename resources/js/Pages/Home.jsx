import { Link } from "@inertiajs/react";
import React, { useState } from "react";
import ChatBackground from "../../assets/ChatBackground.png";
import DownArrow from "../../assets/DownArrow";

const Home = () => {
    const [activeFAQ, setActiveFAQ] = useState(null);
    return (
        <div className="bg-[#2A2A2A] text-white overflow-hidden">
            <div className="flex items-center justify-between xl:px-[70px] md:px-[30px] px-[15px] py-[30px]">
                <div>
                    <h1 className="uppercase font-bold text-xl">AnimeNook</h1>
                </div>
                <div className="lg:block hidden">
                    <ul className="flex items-center gap-[50px] uppercase text-sm font-semibold">
                        <li>
                            <Link>Home</Link>
                        </li>
                        <li>
                            <Link>Pricing</Link>
                        </li>
                        <li>
                            <Link>Features</Link>
                        </li>
                        <li>
                            <Link>Help</Link>
                        </li>
                    </ul>
                </div>
                <div className="flex items-center">
                    <Link className="border-2 border-white lg:block hidden px-[30px] py-[12px] uppercase font-black text-sm">
                        Get Started
                    </Link>
                    <div className="lg:hidden block">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="32"
                            height="32"
                            viewBox="0 0 24 24"
                        >
                            <path
                                fill="currentColor"
                                d="M3 18v-2h18v2zm0-5v-2h18v2zm0-5V6h18v2z"
                            />
                        </svg>
                    </div>
                </div>
            </div>
            <div className="xl:px-[70px] xl:mt-0 mt-[40px] md:px-[30px] px-[15px]">
                <div className="flex lg:flex-row flex-col items-center xl:px-[50px]">
                    <div className="xl:basis-[45%] flex flex-col lg:items-start items-center basis-[50%] relative">
                        <h1 className="xl:text-7xl lg:text-start text-center md:text-6xl text-5xl w-[90%] xl:font-extrabold font-bold">
                            Get Website To Post Anime And Manga
                        </h1>
                        <img
                            className="absolute xl:top-[-20px] top-[-30px] xl:right-[50px] right-[-10px] w-[50px]"
                            src="https://short.io/static/star-d38c032d63bee9674f56de0a6cb578e7.svg"
                            alt=""
                        />
                        <img
                            className="absolute xl:top-[48%] top-[40%]  w-[300px] lg:left-[70px] md:left-[150px]"
                            src="data:image/svg+xml;base64,"
                            alt=""
                        />
                        <p className="mt-[80px] xl:w-[50%] lg:w-[80%] w-full lg:text-start text-center leading-[1.8] text-[#868686] text-lg font-semibold">
                            Modern, personalize, and share fully branded own
                            website.
                        </p>
                        <button className="bg-white text-black mt-[20px] px-[30px] py-[12px] uppercase font-black text-sm">
                            Start For Free
                        </button>
                    </div>
                    <div className="xl:basis-[55%] basis-[50%]">
                        <img
                            className="lg:w-full h-full md:w-[80%] w-full mx-auto"
                            src="https://short.io/static/hero-dark-80b593ef4062c08bb3a9ddfa8ecf841d.avif"
                            alt=""
                        />
                    </div>
                </div>
            </div>
            <div className="xl:px-[70px] xl:mt-0 mt-[40px] md:px-[30px] px-[15px]">
                <div className="flex lg:flex-row flex-col-reverse items-center xl:px-[50px]">
                    <div className="basis-[50%]">
                        <img
                            className="lg:w-full h-full md:w-[80%] mx-auto lg:mt-0 mt-[50px]"
                            src="https://short.io/static/d03c78145c9802e4805b332af3d8fee7/eacf1/integrations-dark.avif"
                            alt=""
                        />
                    </div>
                    <div className="basis-[50%] flex flex-col lg:text-start text-center lg:items-start items-center">
                        <h1 className="xl:text-7xl md:text-6xl text-4xl xl:font-black font-bold">
                            Make your work easier with us
                        </h1>
                        <p className="pt-[30px] mb-[70px] font-semibold leading-[1.8] text-[#C0C0C0]">
                            Animenook is designed to simplify the workflow of
                            businesses. Animenook binds internal operations to
                            the integrations to automate manual processes. While
                            you are engaged in advanced tasks, the integrations
                            work for you.
                        </p>
                        <button className="bg-white text-black mt-[20px] px-[30px] py-[12px] uppercase font-black text-sm">
                            Get Started
                        </button>
                    </div>
                </div>
            </div>
            <div className="xl:px-[70px] md:px-[30px] px-[15px] mt-[100px]">
                <div className="flex items-center xl:px-[50px] relative">
                    <div className="basis-[25%] lg:relative absolute lg:top-0 md:top-[-100px] top-[-80px] lg:left-0 md:left-[-60px] left-[-40px]">
                        <img
                            className="lg:w-full h-full md:w-[200px] w-[150px]"
                            src="https://short.io/static/644cee883ded284661d5f155ed0ce389/a2f3f/medium-01-dark.avif"
                            alt=""
                        />
                    </div>
                    <div className="lg:basis-[50%] basis-[100%]">
                        <div className="relative">
                            <h1 className="md:text-6xl text-5xl text-center xl:font-black font-bold">
                                Create personalized Website
                            </h1>
                            <img
                                className="absolute xl:top-[30%] leading-[2.3] md:right-0 right-[30px] md:w-[400px] w-[300px]"
                                src="data:image/svg+xml;base64,"
                                alt=""
                            />
                        </div>
                        <p className="py-[50px] text-2xl font-bold text-center">
                            And transfer links from one domain to another.
                        </p>
                        <div className="flex justify-center">
                            <button className="bg-white text-black px-[30px] py-[12px] uppercase font-black text-sm">
                                EXPLORE PRICING PLAN
                            </button>
                        </div>
                        <p className="mt-[30px] font-semibold text-center">
                            7-day free trial for paid plans & unlimited
                            redirects in each plan.
                        </p>
                    </div>
                    <div className="basis-[25%] lg:relative absolute lg:bottom-0 md:bottom-[-60px] bottom-[-100px] lg:right-0 md:right-[-80px] right-[-90px]">
                        <img
                            className="lg:w-full h-full md:w-[200px] w-[150px]"
                            src="https://short.io/static/9e9c5ea80909c16b085032d4ace52fed/d19d9/medium-02-dark.avif"
                            alt=""
                        />
                    </div>
                </div>
            </div>
            <div className="xl:px-[70px] md:px-[30px] px-[15px] mt-[100px]">
                <div className="flex xl:px-[50px] lg:flex-row flex-col items-center">
                    <div className="xl:basis-[40%] flex flex-col lg:items-start items-center basis-[50%]">
                        <img
                            className="mb-[60px] w-[100px]"
                            src="https://short.io/static/chat-8b5b6f04fb69dbe0a772f7d695503bbb.svg"
                            alt=""
                        />
                        <h1 className="xl:text-6xl text-5xl text-center font-black">
                            Friendly support that cares
                        </h1>
                        <p className="py-[40px] lg:w-full w-[70%] lg:text-start text-center font-medium">
                            We are always here for you and really care about
                            your success in using short links. So you always get
                            answers.
                        </p>
                        <p className="pt-[30px] md:text-start text-center text-lg font-semibold">
                            Get in touch from Monday to Friday to receive a
                            quick response.
                        </p>
                        <div className="flex items-center mt-[30px]">
                            <button className="bg-white border-2 border-transparent px-[30px] py-[12px] uppercase text-black font-black text-sm">
                                Chat Now
                            </button>
                            <button className="border-2 border-white px-[30px] py-[12px] uppercase font-black text-sm">
                                Send Mail
                            </button>
                        </div>
                    </div>
                    <div className="xl:basis-[60%] basis-[50%] lg:mt-0 mt-[60px]">
                        <div className="relative xl:pl-[70px]">
                            <img
                                className="lg:w-full w-[80%] mx-auto"
                                src={ChatBackground}
                                alt=""
                            />
                            <img
                                className="absolute lg:w-[300px] w-[250px] top-[50%] lg:left-[60%] left-[50%] translate-x-[-50%] translate-y-[-50%]"
                                src="https://short.io/static/85011b3503946de260c65b7d5d4e8206/f6628/support-worker-dark.avif"
                                alt=""
                            />
                        </div>
                    </div>
                </div>
            </div>
            <div className="xl:px-[70px] md:px-[30px] px-[15px] mt-[100px] bg-[#383738]">
                <div className="xl:px-[250px] lg:px-[130px]  py-[100px] relative">
                    <img
                        className="absolute lg:block hidden xl:w-[200px] w-[150px] xl:left-0 left-[-30px] top-[15%]"
                        src="https://short.io/static/medium-03-dark-ac6c807c37be4f5cf84137be0575ac9a.png"
                        alt=""
                    />
                    <img
                        className="absolute lg:block hidden bottom-[15%] xl:w-[250px] w-[150px] xl:right-[-50px] right-[-30px]"
                        src="https://short.io/static/medium-02-dark-15f9c233500faeac754842895de4a1dc.png"
                        alt=""
                    />
                    <div className="flex items-center justify-between">
                        <h1 className="md:text-6xl text-5xl font-black">FAQ</h1>
                        <button className="border-2 border-white px-[30px] py-[12px] uppercase font-black text-sm">
                            VISIT HELP CENTER
                        </button>
                    </div>
                    <div className="flex flex-col gap-[30px] mt-[60px]">
                        <div
                            onClick={() =>
                                setActiveFAQ((prev) => (prev === 0 ? null : 0))
                            }
                            className={`bg-[#2A2A2A] ${
                                activeFAQ === 0
                                    ? "h-[250px]"
                                    : "md:h-[110px] h-[70px]"
                            } md:px-[70px] px-[40px] md:py-[40px] py-[20px] overflow-hidden transition-all`}
                        >
                            <div className="flex  cursor-pointer items-center justify-between">
                                <p className="text-lg font-semibold">
                                    How To Buy A Domain
                                </p>
                                <DownArrow />
                            </div>
                            <p className="mt-[30px] leading-[1.9] text-sm font-bold">
                                Lorem ipsum dolor sit amet consectetur
                                adipisicing elit. Fuga minus temporibus sapiente
                                quis tempore at voluptatum dolore similique.
                                Totam molestiae saepe ullam suscipit. Distinctio
                                doloribus, culpa dolore deleniti excepturi
                                magnam.
                            </p>
                        </div>
                        <div
                            onClick={() =>
                                setActiveFAQ((prev) => (prev === 1 ? null : 1))
                            }
                            className={`bg-[#2A2A2A] ${
                                activeFAQ === 1
                                    ? "h-[250px]"
                                    : "md:h-[110px] h-[70px]"
                            } md:px-[70px] px-[40px] md:py-[40px] py-[20px] overflow-hidden transition-all`}
                        >
                            <div className="flex  cursor-pointer items-center justify-between">
                                <p className="text-lg font-semibold">
                                    How To Buy A Domain
                                </p>
                                <DownArrow />
                            </div>
                            <p className="mt-[30px] leading-[1.9] text-sm font-bold">
                                Lorem ipsum dolor sit amet consectetur
                                adipisicing elit. Fuga minus temporibus sapiente
                                quis tempore at voluptatum dolore similique.
                                Totam molestiae saepe ullam suscipit. Distinctio
                                doloribus, culpa dolore deleniti excepturi
                                magnam.
                            </p>
                        </div>
                        <div
                            onClick={() =>
                                setActiveFAQ((prev) => (prev === 3 ? null : 3))
                            }
                            className={`bg-[#2A2A2A] ${
                                activeFAQ === 3
                                    ? "h-[250px]"
                                    : "md:h-[110px] h-[70px]"
                            } md:px-[70px] px-[40px] md:py-[40px] py-[20px] overflow-hidden transition-all`}
                        >
                            <div className="flex  cursor-pointer items-center justify-between">
                                <p className="text-lg font-semibold">
                                    How To Buy A Domain
                                </p>
                                <DownArrow />
                            </div>
                            <p className="mt-[30px] leading-[1.9] text-sm font-bold">
                                Lorem ipsum dolor sit amet consectetur
                                adipisicing elit. Fuga minus temporibus sapiente
                                quis tempore at voluptatum dolore similique.
                                Totam molestiae saepe ullam suscipit. Distinctio
                                doloribus, culpa dolore deleniti excepturi
                                magnam.
                            </p>
                        </div>
                        <div
                            onClick={() =>
                                setActiveFAQ((prev) => (prev === 4 ? null : 4))
                            }
                            className={`bg-[#2A2A2A] ${
                                activeFAQ === 4
                                    ? "h-[250px]"
                                    : "md:h-[110px] h-[70px]"
                            } md:px-[70px] px-[40px] md:py-[40px] py-[20px] overflow-hidden transition-all`}
                        >
                            <div className="flex  cursor-pointer items-center justify-between">
                                <p className="text-lg font-semibold">
                                    How To Buy A Domain
                                </p>
                                <DownArrow />
                            </div>
                            <p className="mt-[30px] leading-[1.9] text-sm font-bold">
                                Lorem ipsum dolor sit amet consectetur
                                adipisicing elit. Fuga minus temporibus sapiente
                                quis tempore at voluptatum dolore similique.
                                Totam molestiae saepe ullam suscipit. Distinctio
                                doloribus, culpa dolore deleniti excepturi
                                magnam.
                            </p>
                        </div>
                        <div
                            onClick={() =>
                                setActiveFAQ((prev) => (prev === 2 ? null : 2))
                            }
                            className={`bg-[#2A2A2A] ${
                                activeFAQ === 2
                                    ? "h-[250px]"
                                    : "md:h-[110px] h-[70px]"
                            } md:px-[70px] px-[40px] md:py-[40px] py-[20px] overflow-hidden transition-all`}
                        >
                            <div className="flex  cursor-pointer items-center justify-between">
                                <p className="text-lg font-semibold">
                                    How To Buy A Domain
                                </p>
                                <DownArrow />
                            </div>
                            <p className="mt-[30px] leading-[1.9] text-sm font-bold">
                                Lorem ipsum dolor sit amet consectetur
                                adipisicing elit. Fuga minus temporibus sapiente
                                quis tempore at voluptatum dolore similique.
                                Totam molestiae saepe ullam suscipit. Distinctio
                                doloribus, culpa dolore deleniti excepturi
                                magnam.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
            <div className="xl:px-[70px] md:px-[30px] px-[15px] xl:pt-[100px] relative bg-[#383738]">
                <div className="py-[60px] flex flex-col items-center">
                    <img
                        className="absolute lg:block hidden xl:w-[350px] w-[200px] xl:bottom-[20px] bottom-[60px] left-[50px]"
                        src="https://short.io/static/433f83e293bcc39501b67de9dd850269/19d4f/medium-05-dark.avif"
                        alt=""
                    />
                    <img
                        className="absolute lg:block hidden xl:w-[300px] w-[200px] xl:bottom-[20px] bottom-[60px] right-[50px]"
                        src="https://short.io/static/09d857b577dd4e89ef4b8c72c98363b5/10c2b/medium-06-dark.avif"
                        alt=""
                    />
                    <h1 className="xl:text-6xl md:text-start text-center text-5xl xl:font-black font-bold">
                        It's easy to get started
                    </h1>
                    <p className="py-[60px] md:text-start text-center text-2xl font-black">
                        And it's free — two things everyone loves.
                    </p>
                    <button className="bg-white text-black px-[30px] py-[15px] uppercase font-black text-sm">
                        Get Started
                    </button>
                </div>
            </div>
            <div className="xl:px-[70px] px-[15px] mt-[100px]">
                <div className="xl:px-[50px]">
                    <div className="grid lg:grid-cols-6 md:grid-cols-4 grid-cols-1 lg:gap-y-0 gap-y-[30px]">
                        <div className="md:col-span-2 ">
                            <h1 className="text-2xl font-black">AnimeNook</h1>
                            <h1 className="text-3xl font-black xl:mt-[40px] mt-[20px]">
                                info@animenook.io
                            </h1>
                            <img
                                className="w-[150px] xl:my-[40px] my-[20px]"
                                src="https://short.io/static/medium-footer-b20387b06981aa3b5fcadea07c170ef3.avif"
                                alt=""
                            />
                            <div className="flex items-center gap-[10px]">
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="25"
                                    height="25"
                                    viewBox="0 0 24 24"
                                >
                                    <path
                                        fill="currentColor"
                                        d="M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12c0 4.84 3.44 8.87 8 9.8V15H8v-3h2V9.5C10 7.57 11.57 6 13.5 6H16v3h-2c-.55 0-1 .45-1 1v2h3v3h-3v6.95c5.05-.5 9-4.76 9-9.95"
                                    />
                                </svg>
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="25"
                                    height="25"
                                    viewBox="0 0 24 24"
                                >
                                    <path
                                        fill="currentColor"
                                        d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10s10-4.48 10-10S17.52 2 12 2m4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19c-.14.75-.42 1-.68 1.03c-.58.05-1.02-.38-1.58-.75c-.88-.58-1.38-.94-2.23-1.5c-.99-.65-.35-1.01.22-1.59c.15-.15 2.71-2.48 2.76-2.69a.2.2 0 0 0-.05-.18c-.06-.05-.14-.03-.21-.02c-.09.02-1.49.95-4.22 2.79c-.4.27-.76.41-1.08.4c-.36-.01-1.04-.2-1.55-.37c-.63-.2-1.12-.31-1.08-.66c.02-.18.27-.36.74-.55c2.92-1.27 4.86-2.11 5.83-2.51c2.78-1.16 3.35-1.36 3.73-1.36c.08 0 .27.02.39.12c.1.08.13.19.14.27c-.01.06.01.24 0 .38"
                                    />
                                </svg>
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="25"
                                    height="25"
                                    viewBox="0 0 24 24"
                                >
                                    <path
                                        fill="currentColor"
                                        d="M16.6 5.82s.51.5 0 0A4.278 4.278 0 0 1 15.54 3h-3.09v12.4a2.592 2.592 0 0 1-2.59 2.5c-1.42 0-2.6-1.16-2.6-2.6c0-1.72 1.66-3.01 3.37-2.48V9.66c-3.45-.46-6.47 2.22-6.47 5.64c0 3.33 2.76 5.7 5.69 5.7c3.14 0 5.69-2.55 5.69-5.7V9.01a7.35 7.35 0 0 0 4.3 1.38V7.3s-1.88.09-3.24-1.48"
                                    />
                                </svg>
                            </div>
                        </div>
                        <div>
                            <h1 className="text-lg font-bold">Product</h1>
                            <ul className="flex text-sm font-black flex-col gap-[15px] mt-[30px]">
                                <li>
                                    <Link>Features</Link>
                                </li>
                                <li>
                                    <Link>Apps And Integration</Link>
                                </li>
                                <li>
                                    <Link>Pricing</Link>
                                </li>
                            </ul>
                        </div>
                        <div>
                            <h1 className="text-lg font-bold">Resources</h1>
                            <ul className="flex text-sm font-black flex-col gap-[15px] mt-[30px]">
                                <li>
                                    <Link>API For Developer</Link>
                                </li>
                                <li>
                                    <Link>FAQ</Link>
                                </li>
                                <li>
                                    <Link>System Status</Link>
                                </li>
                            </ul>
                        </div>
                        <div>
                            <h1 className="text-lg font-bold">About us</h1>
                            <ul className="flex text-sm font-black flex-col gap-[15px] mt-[30px]">
                                <li>
                                    <Link>Our Team</Link>
                                </li>
                                <li>
                                    <Link>About Company</Link>
                                </li>
                                <li>
                                    <Link>Contact Us</Link>
                                </li>
                            </ul>
                        </div>
                        <div>
                            <h1 className="text-lg font-bold">Community</h1>
                            <ul className="flex text-sm font-black flex-col gap-[15px] mt-[30px]">
                                <li>
                                    <Link>Blogs</Link>
                                </li>
                                <li>
                                    <Link>Help Center</Link>
                                </li>
                                <li>
                                    <Link>Resource Abuse</Link>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div className="h-[0.4px] bg-white my-[30px]"></div>
                    <div className="flex md:flex-row flex-col justify-between md:gap-0 gap-[30px] md:items-center pb-[30px]">
                        <p className="text-[12px]">
                            © Copyright 2015-2024 Short.cm Inc. All Rights
                            Reserved.
                        </p>
                        <div className="flex gap-[30px] text-[12px]">
                            <p>Terms & Conditions</p>
                            <p>Privacy Policy and GDPR</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default Home;
